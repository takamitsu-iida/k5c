...
単にAPIを利用するだけならマニュアルに記載されている通りにcurlコマンドを実行すればいいと思います。
...
検証時には作っては削除して、を繰り返すと思います。
この手の操作は簡単にできるようにした方がいいですね。

ルールを一括で作成するにはこうします。エクセルファイルにあるIDが空白になっているルールを全て作成します。
...
- 仮想サーバに割り当てられたポートのMACアドレス、IPアドレスの組み合わせ以外の通信を遮断するフィルタが自動設定されます。→飯田注：VRRPやHSRPのような仮想IP/仮想MACを使うものは、ポートに追加設定が必要、ということです。Microsoft NLBの利用は禁止されています。
