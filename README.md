...
ネットワークコネクタを経由しないと通信できない通信をスタティックで追加設定します。
イントラネット向けの通信なので、プライベートIPアドレスを集約して、ネットワークコネクタに向けておけば無難でしょう。
...
コネクタエンドポイントはルータみたいなものですが、そのルーティングを操作するためのAPIは公開されていません。
...
-net01(10.1.2.0/24)はコネクタエンドポイントにスタティックルーティングを追加しないと到達できない経路です。
アドレスとゲートウェイをセットにして事業者に伝えればいいでしょう。
（申請しないと、事業者は知り得ない情報です）

したがって、事業者への申請書には以下の情報が必要になると思われます。

- 作成したネットワークコネクタのID(物理接続する対象を識別)
...
だいたいの場合、プライベートIPアドレスを集約して伝えれば問題ないでしょう。
