...
パラメータシートを読み取ってネットワークを構成したり、稼働している環境からパラメータシートを起こしたりするなら、Pythonスクリプトを駆使するといいと思います。
...
K5のネットワークを構成する要素は、作成と同時に接続するものと、別々に作成したものを後からくっつけるものがあります。
構成要素の親子関係を理解することが大事です。
...
ネットワークとサブネットは1:1の関係になり、器となるネットワークを作成した後、そのIDを指定してサブネットを作成します。したがって、サブネットは作成と同時に親子関係を作り、単独で存在することはありません。
...
> セキュリティグループにおいては、169.254.169.254/32 tcp/80の通信がポートから出ていけるように許可しないといけませんが、この通信はファイアウォールには着信しないと思いますので、特にケアする必要はないと思います。
...
- **ネットワークコネクタは1階層ネットワークのみ利用可能**　（★１）要補足説明
...
- 仮想サーバから仮想ルータを経由して「http://169.254.169.254」との通信ができるよう、セキュリティグループとルーティングを設定する
...
（構内接続と同じように、事業者に申請すればいいんじゃね？という気もしますが）

![fig180](https://cloud.githubusercontent.com/assets/21165341/26231168/9bc7fb7a-3c88-11e7-9281-89c751cd6256.png)


## [機能説明書](https://k5-doc.jp-east-1.paas.cloud.global.fujitsu.com/doc/jp/iaas/document/k5-iaas-function-manual.pdf) A.1 制限値
