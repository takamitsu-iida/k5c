...
K5のネットワークを構成する要素は、作成と同時に接続するものと、別々に作成して後からくっつけるものがあります。
どの順番で何を作っていくべきか、その手順を考えるには構成要素の親子関係を理解することが大事です。
...
ネットワークはサブネットやポートを紐付けるための器になります。
器なので真っ先に作成することになります。
ネットワーク自身に設定される情報は多くはありません（どのAZに作るか、ということと、あとは名前くらいです）。

ネットワークとサブネットは1:1の関係になり、器になるネットワークを作成した後、そのIDを指定してサブネットを作成します。したがって、サブネットは作成と同時に親子関係を作り、サブネットだけが単独で存在することはありません。

OpenStackのネットワークは基本的にDHCPありきで動作します。仮想サーバに固定IPを割り当てる場合でも、アドレスの配布自体はDHCPで行います。DHCPに関連した情報もサブネットの中に設定します。DNSサーバの情報や、仮想サーバに追加のルーティング情報を配布したい場合も、サブネットの中に設定します。ですので、サブネットが事実上の『ネットワークの本体』と考えて良さそうです。

> NOTE:
>
> サブネットを削除することもできますが、その場合はDHCP機能が巻き添えを食うようです。
> サブネットを削除したら、親になるネットワークも作り直さないといけません。
...
> NOTE:
>
> IPアドレスは仮想サーバではなく、ポートに割り当てられる点に注意。
> 仮想サーバにもIPアドレスの設定がありますが、それをいじると悲劇が起こるようです。
...
少々ややこしいのは、データセンター構内で別の物理ネットワークと接続する場合です。
ネットワークコネクタという聞きなれない用語が登場します。
...
　　　　├─Availability Zone
　　　　│　　│
　　　　│　　└─コネクタエンドポイント(network_connector_endpoint_id)
　　　　│　　　　│
　　　　│　　　　└─ポート(port_id)
　　　　│
　　　　│
...
K5の事業者側のインフラにはネットワークコネクタプールというものが存在します。
これは最初から存在するもので、利用者は変更できません。

ネットワークコネクタプールの中にネットワークコネクタを作成します。
これが外部との接点になります。
外側の接続設定はK5事業者が行いますので、（申請書を書くとか）何らかの形で依頼をすることになると思います。

ネットワークコネクタを作成するときにアベイラビリティゾーンを指定することはできません。
すなわちネットワークコネクタはアベイラビリティゾーンの外側にあり、事業者側のインフラの一部、と考えたほうがいいでしょう。

作成したネットワークコネクタにはコネクタエンドポイントを接続します。これが内側との接点になります。
コネクタエンドポイントとポートを紐付けることで内側のネットワークと接続します。

ネットワークコネクタには複数のコネクタエンドポイントを作成できます。
したがって一つの構内接続を複数のプロジェクトで共有することもできます。

以上から、外部との接続を作成する順番はこのようになります。

1. 事業者のネットワークコネクタプールのIDを調べる
1. そのプールのIDを指定して、ネットワークコネクタを作成する
1. 作成したネットワークコネクタのIDを調べる
1. そのネットワークコネクタのIDを指定してコネクタエンドポイントを作成する
1. 事前に作成しておいたポートのオーナーとしてコネクタエンドポイントを指定する
...
1. ネットワークコネクタを作成する
1. コネクタエンドポイントを作成する
1. コネクタエンドポイントとポートを接続する
1. ルータと外部ネットを接続する
1. サブネットのルーティング設定(DHCP設定)
1. ルータのルーティング設定
1. ネットワークコネクタへのルーティング設定
1. ファイアウォールルールの作成
1. ファイアウォールポリシーの作成と変更と削除
1. ルータにファイアウォールを紐付け
1. フローティングIPの作成

長い手順ですが、ゼロからの作成であればあまり悩むことはないと思います。
むしろ途中までできてる環境を渡されて『ネットワークコネクタを追加で作成してください』なんてことを言われた方が面倒です。
...
> NOTE:
>
> DHCPを有効にしたネットワーク(サブネット)にポートを作成すると、device_ownerが network:dhcp になっている名無しのポートが作られます。
> ポートを一覧表示すると、作った数よりも多くのポートが見えることになります。
> この名無しのポートがそのセグメントにおけるDHCPサーバの役割をするのだと思います。
...
> NOTE:
>
> ファイアウォールを通過したセッションのログはどうやって確認したらいいのか、公開資料ではみつけられませんでした。

<BR>

K5のネットワークではファイアウォールの利用が推奨となっています。セキュリティグループ機能では『送信元』を指定しての許可、遮断ができません。なので仮想ルータでファイアウォールを使い、サーバのポートでセキュリティグループを設定する、という多層防衛が推奨されます。

K5のファイアウォールはネットワークを構成する要素ではなく、ルータに付与される『通信制限の機能』です。
ファイアウォールでセグメントを分割する、といったことはできません。

一般的なファイアウォール製品は物理ポートやセキュリティグループ、
inside/outsideといったアイデアを駆使して壁を作っていきますが、
K5のファイアウォールにはそれがなく、物理ポートを意識しない作りになっています。

ファイアウォールには暗黙のdenyが入っていますので、適用すると全通信が遮断されます。この状態から必要な通信を許可していくわけですが、一定の制約事項を導入しないと設計が大変です。
...
> でも、IPsec通信を許可する、という場合はどうするんだろう。
> ESPはUDPでもTCPでもなく、プロトコルそのものがESPなのだが。

<BR>

> NOTE:
>
> 外部からフローティングIP宛てに着信する通信を許可したい場合、対応するプライベートアドレスの方を使ってルールを書きます。NATが先に処理されるので、変換後のアドレスでルールを書くということです。
>
> 情報ソース： クラウドデザインパターンのセキュリティグループ/ファイアーウォール併用パターン

<BR>

> NOTE:
>
> アプリケーションインスペクションの機能はありませんので、FTPの通信を許可するような場合は、難しいルール作りが必要です。
>
> 情報ソース： FUJITSU Cloud Service K5 FAQ

<BR>

> NOTE:
>
> セキュリティグループにおいては、宛先169.254.169.254/32, tcp/80の通信がポートから出ていけるように許可しないといけませんが、この通信はファイアウォールには着信しないと思いますので、特にケアする必要はないと思います（穴あけ不要だと思っています）。
...
--nameオプションに'all'を指定すると、エクセルファイル中のIDが空白になっているルールを全て作成します。--saveオプションを付けて作成したIDをエクセルに反映させたほうが楽できます。
...
bin/k5-create-floatingip.py コマンドを使います。
...
この例では外から見た133.162.215.230は内側の10.1.1.100と１対１に対応します。

> NOTE:
>
> APIを発行してから実際にフローティングIPが使えるようになるまでにしばらく時間がかかります。
> なんでstatusがDOWNなんだろ、と思わずに、気長に待ちましょう。
...
## [FUJITSU Cloud Service K5 FAQ](http://jp.fujitsu.com/solutions/cloud/k5/faq/)


- K5から外に出ていったときに使われる送信元IP(グローバルIP)を確認する手段はない(実際に通信してみないと分からない)

- ダイナミックポートアプリケーション対応機能はない(FTPとか)

- 仮想ルータではNAT処理が先に行われ、そのIPを元にファイアウォールでフィルタリングされる

- ロードバランサから分散対象サーバに通信する際の送信元IPアドレスはロードバランサのアドレス

- ファイアウォール/セキュリティグループはIngressで許可された通信の応答はEgressのルールによらず通信可能。逆も同様。
...
Windowsのコマンドプロンプトでも実行できますが、画面の横幅を広げておかないと見苦しい表示になってしまいます。コマンドプロンプトのプロパティから、レイアウト→ウィンドウのサイズで幅を140以上にするのがおすすめです。
