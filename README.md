...
- iida-az1-net01はルータ1台のセグメントなので、ルーティングの設定は不要（デフォルトルートに従う）
...
ルータから見た時、-net01(10.1.1.0/24)と-net02(10.1.2.0/24)は自足に接続していますので、Locally connected経路です。これらについては設定不要です。ネットワークコネクタを経由しないと通信できない通信をスタティックで設定します。

プライベートIPアドレス行きは全てネットワークコネクタに向けておけば無難でしょう。
...
ここでの例では、K5の中にある経路は-net01(10.1.1.0/24)と-net02(10.1.2.0/24)なので、
これら経路を広告してもらうように申請書に書きます。

申請書には以下の情報が必要になると思われます。

- 作成したネットワークコネクタのID
- K5から外部に広告する経路(この例では10.1.1.0/24と10.1.2.0/24)
- コネクタエンドポイントが接続しているポートのID
