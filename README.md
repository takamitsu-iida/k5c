...
パラメータシートを読み取ってコマンド一発でネットワークを構成するなら、Pythonスクリプトを使うといいと思います。
...
## サブネットのルーティング設定(DHCP設定)

同じネットワーク（サブネット）にルータが2台いる場合、どの宛先はどっち、という判断が必要です。VMのルーティングテーブルを手動で変更してもいいのかもしれませんが、OpenStackではDHCPのオプションコード121（RFC3442 : Classless Static Route Option for DHCPv4）を使ってVMに伝えます。経路を追加、削除した場合はVMを再起動しないと伝搬しません。

ここまでの例では、

- iida-az1-net01はルータ1台のセグメントなので、ルーティングの設定は不要
- iida-az1-net02はルータとコネクタエンドポイントがいるので、ルーティングの設定が必要

ということになります。

iida-az1-net02からみたとき、-net01(10.1.1.0/24)はルータ向け、その他の10系のアドレス(10.0.0.0/8)はネットワークコネクタ向け、となります。
...
サブネットに設定した経路情報をルータが自動で拾ってくれるのか、は疑問です。

> スタティックルートの設定が必要だと思っていますが、どの資料をみても、必要だとは書いてない・・・

ルータから見た時、-net01(10.1.1.0/24)と-net02(10.1.2.0/24)は自足に接続していますので、Locally connected経路です。これらについては設定不要です。ネットワークコネクタを経由しないと通信できない通信をスタティックで設定します。宛先10.0.0.0/8をネットワークコネクタに向けます。

<BR>

## ネットワークコネクタへのルーティング設定
...
ここでの例では、K5の中にある経路は-net01(10.1.1.0/24)と-net02(10.1.2.0/24)なので、これら経路を広告してもらうように申請書に書きます。

### 外部方向→K5

逆に外部ネットワークで使っている経路情報をeBGPで伝えます。
eBGPで伝えられる経路情報の数には制限があり、上限200経路とされています。
うまく集約をかけて不要な経路を流さないように工夫が必要です。
ここまでの例では10.0.0.0/8だけを伝えれば十分でしょう。

> K5のネットワークコネクタにBGPで教えた経路は、どこまで伝わるのかしら？
